...
module.exports = async function weatherReport(msg) {
    if ((msg.content).toLowerCase().includes("!prognoza")) {
        ...
        try {
            ...
            const day1 = {
                date: `Day: ${days[day]}`,
                morning: `${Math.round(forecastData.data.daily[0].temp.morn)}°C`,
                day: `${Math.round(forecastData.data.daily[0].temp.day)}°C`,
                description: forecastData.data.daily[0].weather[0].description,
            }
            const day2 = {
            ...
            const embededMessage = new Discord.MessageEmbed()
                ...
                .addFields(
                    { name: day1.date, value: `Ujutro: ${day1.morning}\n Popodne: ${day1.day}\n Opis: ${day1.description}` },
